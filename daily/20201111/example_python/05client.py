...
async def run():
    async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
        res = await client.post("/services/xxx/users", json={"name": "foo"})
        print(res, res.json())

        req = client.build_request("POST", "/services/xxx/users", json={"name": "foo"})
        res = await client.send(req) # auth,allow_redirects,timeout,stream
        print(res, res.json())
