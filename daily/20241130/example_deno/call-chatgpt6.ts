...
export function buildFetchFunctionForOpenAIAPI(options: { apiKey: string, baseUrl?: string, debug?: boolean, fetch?: Fetch }): Fetch {
    ...
    return async function fetch(url: Parameters<Fetch>[0], init?: Parameters<Fetch>[1]): ReturnType<Fetch> {
        init = init ?? {};
