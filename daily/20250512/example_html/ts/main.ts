// ts/main.ts
import { CategoryTemplates, HIERARCHICAL_TEMPLATES, Template } from "./data";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const TEMPLATE_DESCRIPTION_ID = "prompt-template-description";
  ...
  const handleCategorySelect = (categoryName: string) => {
    ...
    const category = HIERARCHICAL_TEMPLATES.find((c) =>
      c.categoryName === categoryName
    );
    if (category) {
      templateListRenderer.renderTemplateList(
  ...
  // 初期表示: カテゴリ一覧
  templateListRenderer.renderCategoryList(
    HIERARCHICAL_TEMPLATES,
    handleCategorySelect,
  );

  // 戻るボタンのイベントリスナー
  document.getElementById(BACK_TO_CATEGORY_LIST_BUTTON_ID)!.addEventListener(
    "click",
    () => {
      viewManager.showCategoryListView();
    },
  );
  document.getElementById(BACK_TO_TEMPLATE_LIST_BUTTON_ID)!.addEventListener(
    "click",
    () => {
      if (currentSelectedCategoryName) {
        // 再度テンプレートリストを表示するためにカテゴリデータを渡す必要がある
        const category = HIERARCHICAL_TEMPLATES.find((c) =>
          c.categoryName === currentSelectedCategoryName
        );
        if (category) {
          templateListRenderer.renderTemplateList(
            category.categoryName,
