...
export class PromptFormRenderer {
  ...
  constructor(
    ...
    this.templateBodyPreviewElement = document.getElementById(templateBodyPreviewId)!;
    this.variableInputsContainer = document.getElementById(variableInputsContainerId)!;
    this.generatedPromptTextarea = document.getElementById(generatedPromptTextareaId)! as HTMLTextAreaElement;
    this.copyButton = document.getElementById(copyButtonId)! as HTMLButtonElement;
  ...
  renderPromptForm(template: Template): void {
    ...
    template.variables.forEach(variableName => {
      ...
      input.addEventListener('input', () => this.updateGeneratedPrompt(template)); // 入力時にプレビュー更新
    ...
    this.copyButton.onclick = async () => {
      if (this.generatedPromptTextarea.value) {
        try {
          await navigator.clipboard.writeText(this.generatedPromptTextarea.value);
          this.feedbackMessageElement.textContent = 'クリップボードにコピーしました！';
  ...
  private updateGeneratedPrompt(template: Template): void {
    ...
    template.variables.forEach(variableName => {
      const inputElement = document.getElementById(`var-${variableName}`) as HTMLInputElement;
      if (inputElement) {
        ...
        // 正規表現でグローバル置換 (gフラグ)
        prompt = prompt.replace(new RegExp(`\\{\\{${variableName}\\}\\}`, 'g'), value || `{{${variableName}}}`);
      }
    });
