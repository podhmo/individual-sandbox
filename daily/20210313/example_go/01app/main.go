...
func run(dialect, dbname string) error {
	...
	users := []*User{
		s.NewUser(&User{Name: "foo"}),
		s.NewUser(&User{Name: "bar"}),
		s.NewUser(&User{Name: "boo"}),
	}
	if _, err := m.InsertValues(ctx, UserTable, users); err != nil {
...
func (m *DBManager) InsertValues(ctx context.Context, info TableInfo, values ...interface{}) (sql.Result, error) {
	r, err := m.DB.NamedExecContext(
		ctx,
		m.Transcriptor.InsertStmt(info),
		values,
	)
	if err != nil {
...
func (s *DBSession) NewUser(user *User) *User {
	user.TenantID = s.Tenant.ID
	return user
}

func NewDBSession(ctx context.Context, m *DBManager, name string) (*DBSession, error) {
	root := Tenant{Name: name}
	r, err := m.InsertValues(ctx, TenantTable, &root)
	if err != nil {
	...
	return &DBSession{DBManager: m, Tenant: root}, nil
}
