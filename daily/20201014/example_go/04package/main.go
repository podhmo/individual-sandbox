...
func (p *Package) NewFile(name string) *File {
	...
	f := &File{Name: name, visitor: p.Visitor, extractor: p.Extractor}
...
func (f *File) AppendStruct(ob interface{}) error {
	v := f.visitor
	s := f.extractor.Extract(ob)
...
func main() {
	...
	pkg := NewPackage("main")
	file := pkg.NewFile("models.go")

	if err := file.AppendStruct(Person{}); err != nil {
		log.Fatalf("!! %+v", err)
	}

	e := &Emitter{Visitor: pkg.Visitor} // xxx
	e.Emit(os.Stdout)
}
