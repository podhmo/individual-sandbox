...
func (c *Converter) ConvertGroup(src *Group) (*GroupView, error) {
	return c.convertGroupNode(&GroupNode{src})
}

func (c *Converter) convertGroupNode(src *GroupNode) (*GroupView, error) {
	if src == nil {
		return nil, errors.Wrap(ErrNotFound, "convert group")
	}
	dst := &GroupView{
		Name: src.Name,
	}
	...
	for i := range src.Members {
		csrc := src.Members[i]
		cdst, err := c.convertPersonNode(&PersonNode{src, &csrc})
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("members[%d]", i))
		}
		dst.Members[i] = *cdst
	}
	return dst, nil
...
func (c *Converter) convertPersonNode(src *PersonNode) (*PersonView, error) {
	if src == nil {
		return nil, errors.Wrap(ErrNotFound, "convert person")
	}
	dst := &PersonView{
		Name: src.Name,
		Age:  src.Age,
