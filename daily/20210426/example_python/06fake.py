...
async def bulk_get(q, *, buffering_time=0.1):
    items = []
    ...
    try:
        await asyncio.wait_for(_bulk_get(), buffering_time)
...
async def worker(client):
    ...
    while True:
        futs = []
        items = await bulk_get(q, buffering_time=0.1)
        if items is END:
            if q.empty():
                break
            continue
        ...
        for action, args in items:
            if action == list_clusters:
                ...
                futs.append(do_list_services(*args))
                ...
                futs.append(do_describe_services(*args))
