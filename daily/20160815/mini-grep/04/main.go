...
var include = flag.String("include", "", "if existed, grepping matched files only")
var exclude = flag.String("exclude", "", "if existed, skipping matched files")
...
func main() {
	...
	default:
		...
		consume := func(filename string) error {
			if includeRx != nil && !includeRx.MatchString(filename) {
				return nil // skipping
			}
			if excludeRx != nil && excludeRx.MatchString(filename) {
				return nil // skipping
			}
