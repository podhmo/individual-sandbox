...
// 問題は
// - requiredに含まれたときだけ、undefinedを許容しないようにしたい
...
interface Options<TStrings extends readonly string[], BStrings extends readonly string[]> {
    string: EnsureLiteralArray<TStrings>
...
): { [key in TStrings[number]]: string } & { [key in BStrings[number]]: boolean } {
...
const args = parseArgs(Deno.args, {
    ...
    boolean: ["color"],
    required: ["name", "color"]
} as const); // as constを付けずともrequiredをしっかりcheckしてほしい OK

console.dir(args, { depth: null });

// string | undefined になると嬉しい
// [deno-ts] Type 'string' is not assignable to type 'never'.
const version : never = args.version; // OK
