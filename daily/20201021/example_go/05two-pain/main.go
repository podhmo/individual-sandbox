...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		if !m.ready {
			...
			m.viewport2 = viewport.Model{Width: msg.Width, Height: msg.Height - verticalMargins}
			m.viewport2.YPosition = headerHeight
			m.viewport2.HighPerformanceRendering = useHighPerformanceRenderer
			m.viewport2.SetContent(m.content)
			m.ready = true

		} else {
			m.viewport.Width = msg.Width
			m.viewport.Height = msg.Height - verticalMargins
			m.viewport2.Width = msg.Width
			m.viewport2.Height = msg.Height - verticalMargins
		}
	...
	m.viewport2, cmd = viewport.Update(msg, m.viewport2)
	if useHighPerformanceRenderer {
