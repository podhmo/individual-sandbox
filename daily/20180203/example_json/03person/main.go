...
func WritePerson(p *Person, w io.Writer) {
	io.WriteString(w, `{`)
	io.WriteString(w, `"id": `)
	io.WriteString(w, fmt.Sprintf(`%d`, p.ID))
	io.WriteString(w, `, "name": `)
	io.WriteString(w, fmt.Sprintf(`%q`, p.Name))
	io.WriteString(w, `, "birthday": `)
	io.WriteString(w, fmt.Sprintf(`%q`, p.Birthday))
...
func WriteVividInfo(v *VividInfo, w io.Writer) {
	...
	io.WriteString(w, fmt.Sprintf(`%q`, v.Color))
	io.WriteString(w, `, "weapon": `)
	io.WriteString(w, fmt.Sprintf(`%q`, v.Weapon))
	io.WriteString(w, `}`)
}
